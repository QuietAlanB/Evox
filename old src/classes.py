...
class Creature:
    def __init__(self, pos, size, angle, color, dir, age, health, hunger, reproduction, traits):
        self.pos = pos
        self.initialPos = pos
        self.color = color
        self.size = size
    ...
    # ======= target mate =======
    def searchMate(self, mate):
        if mate == None: self.mateInfo['mate'] = None
        elif abs(self.pos - mate.pos) < Vector2(self.traits['sight'], self.traits['sight']): 
            if (self.mateInfo['mate'].reproduction['amount'] > self.mateInfo['mate'].reproduction['max'] / 3 and
                self.reproduction['amount'] > self.reproduction['max'] / 3):
    ...
    # ======= move to mate =======
    def mate(self):
        if self.mateInfo['food'] == None: return

        creatureDir = (self.front - self.pos).normalized()
        mateDir = (self.mateInfo['mate'].pos - self.pos).normalized()

        lookAmount = creatureDir.dot(mateDir)
        lookThreshold = 0.95
    ...
    def statsUpdate(self):
        ...
        if self.age['amount'] > self.age['max']: self.dead = True
    ...
    def moveUpdate(self):
        ...
        if self.foodInfo['food'] == None:
            ...
            if not self.turnInfo['turning']:
                self.turnInfo['angle'] = random.randrange(-90, 90, self.traits['speed'])
